...
class AllThreads extends StatefulWidget {
  ...
  const AllThreads(
      {super.key, this.roomId, this.roomsSearch, this.threadsSearch});
...
class AllThreadsController extends State<AllThreads> {
  ...
  void _filterRooms() async {
    setState(() {
      ...
      filtered.sort((a, b) {
        final aHasHighlights = ThreadHighlights().hasHighlightThreads(a.id);
        final bHasHighlights = ThreadHighlights().hasHighlightThreads(b.id);
      ...
      await rooms
        ...
        setState(() {
          ...
          roomThreads.sort((a, b) {
            final aIsHighlight =
                ThreadHighlights().isHighlightThread(roomId, a.eventId);
            final bIsHighlight =
                ThreadHighlights().isHighlightThread(roomId, b.eventId);
  ...
  void setFavoriteThread(String eventId) {
    setState(() {
      ThreadFavorite().setFavorite(
          roomId!, eventId, !ThreadFavorite().isFavorite(roomId!, eventId));
    });
  }
