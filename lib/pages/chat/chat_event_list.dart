  ...
  Widget build(BuildContext context) {
    ...
    for (var i = 0; i < events.length; i++) {
      thisEventsThreadEventCount[events[i].eventId] = events
          .where(
            (_event) =>
                _event.eventId != events[i].eventId &&
                _event.relationshipEventId == events[i].eventId &&
                _event.relationshipType == RelationshipTypes.thread,
          )
          .length;
    }
    ...
    events.removeWhere(
      (event) {
        if (event.type.startsWith('m.call')) {
          ...
          if (allCallEventsByCallId.any((e) =>
      ...
      child: ListView.custom(
        ...
        childrenDelegate: SliverChildBuilderDelegate(
          (BuildContext context, int i) {
            ...
            bool animateIn = animateInEventIndex != null &&
