...
class JitsiDialogState extends State<JitsiDialog> {
  ...
  void getUsers() async {
    ...
    final participants = await Matrix.of(context)
        .client
        .getRoomById(widget.controller.roomId)
        ?.requestParticipants();
  ...
  void initState() {
    ...
    roomNameController.text =
        "${widget.controller.room.name.replaceAll(" ", "")}-${Random().nextInt(100001)}";
    ...
    return AlertDialog.adaptive(
              ...
              Expanded(
                child: ListView.builder(
                  ...
                  itemBuilder: (BuildContext context, int i) =>
                    ...
                    isSelectable: true,
                    onSelect: onSelect,
                    isSelected: selectedUserIds.contains(filteredUsers[i].id),
                  ),
                ),
              ),
      ...
      actions: <Widget>[
        ...
        AdaptiveDialogAction(
          onPressed: () {
            widget.controller
                .sendJitsiRoom(roomNameController.text, selectedUserIds);
            Navigator.of(context, rootNavigator: false).pop();
          },
